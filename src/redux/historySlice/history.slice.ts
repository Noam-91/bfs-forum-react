...
import { getQueriedHistory, recordHistoryView } from './hisotry.thunks.ts';
...
const historySlice = createSlice({
    ...
    extraReducers: (builder) =>{
        ...
        .addCase(getQueriedHistory.rejected, (state, action)=>{
            ...
            // --- recordHistoryView ---
        builder
        .addCase(recordHistoryView.pending, state => {
            state.status = 'loading';
            state.error = null;
        })
        .addCase(
            recordHistoryView.fulfilled,
            (state, action: PayloadAction<string>) => {
            state.status = 'succeeded';
            state.error = null;
            }
        )
        .addCase(recordHistoryView.rejected, (state, action) => {
            state.status = 'failed';
            state.error = (action.payload as IErrorResponse)?.error || action.payload as string || 'Failed to record history';
        })
    }
